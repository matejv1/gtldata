...
Timeline.SecEventPainter = function(params) {
    this._params = params;
...
Timeline.SecEventPainter.prototype.initialize = function(band, timeline) {
    this._band = band;
...
Timeline.SecEventPainter.prototype.getType = function() {
    return 'original';
};

Timeline.SecEventPainter.prototype.addOnSelectListener = function(listener) {
    this._onSelectListeners.push(listener);
};

Timeline.SecEventPainter.prototype.removeOnSelectListener = function(listener) {
...
Timeline.SecEventPainter.prototype.addEventPaintListener = function(listener) {
    this._eventPaintListeners.push(listener);
};

Timeline.SecEventPainter.prototype.removeEventPaintListener = function(listener) {
...
Timeline.SecEventPainter.prototype.getFilterMatcher = function() {
    return this._filterMatcher;
};

Timeline.SecEventPainter.prototype.setFilterMatcher = function(filterMatcher) {
    this._filterMatcher = filterMatcher;
};

Timeline.SecEventPainter.prototype.getHighlightMatcher = function() {
    return this._highlightMatcher;
};

Timeline.SecEventPainter.prototype.setHighlightMatcher = function(highlightMatcher) {
    this._highlightMatcher = highlightMatcher;
};

Timeline.SecEventPainter.prototype.paint = function() {
   ...
   impreciseIconMargin: eventTheme.instant.impreciseIconMargin
    ...
    while (iterator.hasNext()) {
        ...
        if (filterMatcher(evt)) {
			//glavni dogodek za risanje eventa
            this.paintEvent(evt, metrics, this._params.theme, highlightMatcher(evt));
			//this.paintCustomEvent(evt, metrics, this._params.theme, highlightMatcher(evt));
        }
    }
...
Timeline.SecEventPainter.prototype.softPaint = function() {
};

Timeline.SecEventPainter.prototype._prepareForPainting = function() {
...
Timeline.SecEventPainter.prototype.paintCustomEvent = function(evt, metrics, theme, highlightIndex) {
...
Timeline.SecEventPainter.prototype.paintPreciseInstantEvent = function(evt, metrics, theme, highlightIndex) {
...
Timeline.SecEventPainter.prototype.paintImpreciseInstantEvent = function(evt, metrics, theme, highlightIndex) {
    var doc = this._timeline.getDocument();
    var text = evt.getText();
...
Timeline.SecEventPainter.prototype.paintPreciseDurationEvent = function(evt, metrics, theme, highlightIndex) {
    var doc = this._timeline.getDocument();
    var text = evt.getText();
...
Timeline.SecEventPainter.prototype.paintImpreciseDurationEvent = function(evt, metrics, theme, highlightIndex) {
    var doc = this._timeline.getDocument();
    var text = evt.getText();
...
Timeline.SecEventPainter.prototype._encodeEventElID = function(elType, evt) {
    return Timeline.EventUtils.encodeEventElID(this._timeline, this._band, elType, evt);
};

Timeline.SecEventPainter.prototype._findFreeTrack = function(event, rightEdge) {
...
Timeline.SecEventPainter.prototype._paintEventIcon = function(evt, iconTrack, left, metrics, theme, tapeHeight) {
...
Timeline.SecEventPainter.prototype._paintEventLabel = function(evt, text, left, top, width,
    height, theme, labelDivClassName, highlightIndex) {
    var doc = this._timeline.getDocument();
...
Timeline.SecEventPainter.prototype._paintEventTape = function(
...
Timeline.SecEventPainter.prototype._getLabelDivClassName = function(evt) {
    return this._getElClassName('timeline-event-label', evt, 'label');
};

Timeline.SecEventPainter.prototype._getElClassName = function(elClassName, evt, prefix) {
...
Timeline.SecEventPainter.prototype._getHighlightColor = function(highlightIndex, theme) {
    var highlightColors = theme.event.highlightColors;    
    return highlightColors[Math.min(highlightIndex, highlightColors.length - 1)];
};

Timeline.SecEventPainter.prototype._createHighlightDiv = function(highlightIndex, dimensions, theme, evt) {
    var div = null;
    if (highlightIndex >= 0) {
        var doc = this._timeline.getDocument();        
...
Timeline.SecEventPainter.prototype._onClickInstantEventQueryByID = function(icon,domEvt,evt){
...
Timeline.SecEventPainter.prototype._onClickInstantEventByID = function(icon, domEvt, evt) {
...
Timeline.SecEventPainter.prototype._onClickInstantEvent = function(icon, domEvt, evt) {
    var c = SimileAjax.DOM.getPageCoordinates(icon);
    ...
    //this._fireOnSelect(evt.getID());
    //console.log("instant event caceled bubble" + evt.getID());
    domEvt.cancelBubble = false;
    SimileAjax.DOM.cancelEvent(domEvt);
    return false;
};
...
Timeline.SecEventPainter.prototype._onHoverEvent = function(icon, domEvt, evt) {
    var c = SimileAjax.DOM.getPageCoordinates(icon);
    ...
    //this._fireOnSelect(evt.getID());
    console.log("hover event caceled bubble" + evt.getID());
    domEvt.cancelBubble = false;
    //SimileAjax.DOM.cancelEvent(domEvt);
    return false;
};

Timeline.SecEventPainter.prototype._onClickDurationEvent = function(target, domEvt, evt) {
    if ("pageX" in domEvt) {
        var x = domEvt.pageX;
        var y = domEvt.pageY;
    } else {
...
Timeline.SecEventPainter.prototype.showBubble = function(evt) {
...
Timeline.SecEventPainter.prototype._showBubble = function(x, y, evt) {
...
Timeline.SecEventPainter.prototype._fireOnSelect = function(eventID) {
    for (var i = 0; i < this._onSelectListeners.length; i++) {
        this._onSelectListeners[i](eventID);
    }
};

Timeline.SecEventPainter.prototype._fireEventPaintListeners = function(op, evt, els) {
    for (var i = 0; i < this._eventPaintListeners.length; i++) {
